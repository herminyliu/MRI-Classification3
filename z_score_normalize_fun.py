...
def process_csv_file(csv_file_path, is_SC, subdir_path):
    # 如果是_SC.csv文件，进行对数转化
    if is_SC:
        df = pd.read_csv(csv_file_path, header=None, index_col=0)
        df = log_transform(df.astype(float))
    else:
        df = pd.read_csv(csv_file_path, header=0, index_col=0)
    # 标准化数据
    standardized_df = z_score_normalize(df.astype(float))
    # 构建新的文件名
...
def process_folder(folder_path):
    ...
    for root, dirs, files in os.walk(folder_path):
        for subdir in dirs:
            ...
            csv_files = [file for file in os.listdir(subdir_path) if file.endswith('_FUN.csv')]
            for csv_file in csv_files:
                csv_file_path = os.path.join(subdir_path, csv_file)
                # 判断当前文件是否为_SC.csv文件
                is_SC = csv_file.endswith('_SC.csv')
                process_csv_file(csv_file_path, is_SC, subdir_path)


if __name__ == "__main__":
    # !!!注意，这是按行进行的标准化
    process_folder(r"C:\Users\28760\Desktop\脑图大创\24年新\229服务器数据\data229")
